...
> **状态：本文是「方向性构想」，不是可以据此开工的设计。**
>
> 真正已经定稿的接口 —— 内核镜像格式与装载规则、入口 ABI、`BootInfo`、交接语义、退出码 ——
> 在 [kernel_interface_CN.md](kernel_interface_CN.md)。
> 本文其余部分（IPC、能力模型、调度、用户态服务、Wasm/WASI 隔离模型）**尚未设计**，
> 只表达方向与约束；每一项在实现前都需要各自的接口文档。
> 实现的顺序依赖见 [kernel_interface_CN.md](kernel_interface_CN.md) §9。
...
> 以下三项**都还没有接口定义**，也不属于任何已定的里程碑。按当前顺序，它们位于 M5
> （IPC 与能力模型），要等**真实用户态进程存在**之后再来约束设计 —— 见
> [kernel_interface_CN.md](kernel_interface_CN.md) §9。
