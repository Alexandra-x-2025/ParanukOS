...
8. **立刻关闭中断（`cli`）** —— 退出引导服务后固件的中断向量与处理程序都不再有效，遗留的硬件中断会跳进已失效的固件 ISR 并可能破坏内存；
9. 填充 `BootInfo`，其中内存图的指针 / 条目数 / desc_size / desc_ver 取自上面返回的那份内存图 —— 纯内存写入，退出之后仍然安全；
...
> ⚠️ **M1 开发期间的实测现象：** 内核变大、时序改变后，`BootInfo` 的第一个字（4 字节）曾在
> 内核校验它之前被间歇性改写。退出调用与 `cli` 之间的窗口是首要嫌疑，因此上面第 8 步改为
> 退出后立刻关闭中断。该损坏无法稳定复现，所以这是一次**加固而非已证实的修复**；内核的
> `BootInfo` 校验会把这类问题变成明确的错误（39），而不是静默的行为异常。
